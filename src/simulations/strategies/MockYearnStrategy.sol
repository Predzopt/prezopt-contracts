// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;

import {IStrategy} from "../../interfaces/IStrategy.sol";
import {IERC20} from "openzeppelin-contracts/token/ERC20/IERC20.sol";
import {SafeTransferLib} from "solady/src/utils/SafeTransferLib.sol";

contract MockYearnStrategy is IStrategy {
    IERC20 public asset;
    IERC20 public rewardToken;
    
    uint256 public baseAPY = 600; // 6% APY (conservative)
    uint256 public rewardAPY = 100; // 1% reward APY
    uint256 public totalDeposited;
    uint256 public lastUpdateTime;
    uint256 public accruedYield;
    uint256 public managementFee = 200; // 2% management fee

    event YieldAccrued(uint256 amount);
    event RewardsHarvested(uint256 amount);
    event ManagementFeeCharged(uint256 amount);

    constructor(address _asset, address _rewardToken) {
        asset = IERC20(_asset);
        rewardToken = IERC20(_rewardToken);
        lastUpdateTime = block.timestamp;
    }

    function invest(uint256 amount) external override {
        _updateYield();
        
        SafeTransferLib.safeTransferFrom(address(asset), msg.sender, address(this), amount);
        totalDeposited += amount;
    }

    function withdraw(uint256 amount) external override {
        _updateYield();
        
        require(totalDeposited >= amount, "Insufficient balance");
        totalDeposited -= amount;
        
        SafeTransferLib.safeTransfer(address(asset), msg.sender, amount);
    }

    function harvest() external override returns (uint256) {
        _updateYield();
        
        uint256 rewardAmount = (totalDeposited * rewardAPY * (block.timestamp - lastUpdateTime)) / (365 days * 10000);
        
        if (rewardAmount > 0) {
            (bool success,) = address(rewardToken).call(
                abi.encodeWithSignature("mint(address,uint256)", address(this), rewardAmount)
            );
            if (success) {
                emit RewardsHarvested(rewardAmount);
                return rewardAmount;
            }
        }
        
        return 0;
    }

    function estimatedTotalAssets() external view override returns (uint256) {
        uint256 timeElapsed = block.timestamp - lastUpdateTime;
        uint256 grossYield = (totalDeposited * baseAPY * timeElapsed) / (365 days * 10000);
        uint256 fees = (grossYield * managementFee) / 10000;
        uint256 netYield = grossYield - fees;
        return totalDeposited + accruedYield + netYield;
    }

    function _updateYield() internal {
        uint256 timeElapsed = block.timestamp - lastUpdateTime;
        if (timeElapsed > 0 && totalDeposited > 0) {
            uint256 grossYield = (totalDeposited * baseAPY * timeElapsed) / (365 days * 10000);
            uint256 fees = (grossYield * managementFee) / 10000;
            uint256 netYield = grossYield - fees;
            
            accruedYield += netYield;
            lastUpdateTime = block.timestamp;
            
            emit YieldAccrued(netYield);
            emit ManagementFeeCharged(fees);
        }
    }

    function getCurrentAPY() external view returns (uint256) {
        uint256 grossAPY = baseAPY + rewardAPY;
        uint256 fees = (grossAPY * managementFee) / 10000;
        return grossAPY - fees;
    }

    function setAPY(uint256 _baseAPY, uint256 _rewardAPY) external {
        baseAPY = _baseAPY;
        rewardAPY = _rewardAPY;
    }

    function setManagementFee(uint256 _managementFee) external {
        managementFee = _managementFee;
    }
}